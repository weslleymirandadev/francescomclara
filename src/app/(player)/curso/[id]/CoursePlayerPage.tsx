  ...
  return (
    <div className="flex bg-white min-h-screen pt-10 relative">
      <button
        ...
        className="min-[1501px]:hidden fixed bottom-6 right-6 z-60 bg-slate-900 text-white p-4 rounded-2xl shadow-2xl flex items-center gap-2 active:scale-95 transition-transform"
      >
        <FiLayers size={20} />
      ...
      <main className="flex-1 w-full">
        <CourseContent
          ...
          isCompleted={courseData?.completedLessons?.includes(activeLesson?.id)}
        />
      </main>
    </div>
  );
