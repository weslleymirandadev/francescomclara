...
export default function ObjectiveBanner({ objective, currentImg }: BannerProps) {
  ...
  const IconComponent = (GiIcons as any)[objective.icon] || GiIcons.GiAbstract050;
  const isCustomImage = !!currentImg || (!!objective.imageUrl && objective.imageUrl !== "");
  const displayImg = currentImg || objective.imageUrl;
  const mainAngle = objective.iconRotate || 0;
  const badgeAngle = mainAngle * 0.1;

  const changeRotation = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = parseInt(e.target.value);
    await updateObjectiveSettingsAction(objective.id, { iconRotate: val });
  }

  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !objective?.id) return; 

    const reader = new FileReader();
    reader.onloadend = async () => {
      const base64String = reader.result as string;
      await updateObjectiveImageAction(objective.id, base64String);
    };
                ...
                <div className="grid grid-cols-4 gap-4 max-h-72 overflow-y-auto pr-2 custom-scrollbar">
                  {(searchResults.length > 0 ? searchResults : QUICK_SUGGESTIONS).map(iconName => (
                    <button 
                      key={iconName}
                      onClick={async () => {
                        await updateObjectiveSettingsAction(objective.id, { icon: iconName });
                        setIsOpen(false);
                        setSearch("");
                      }}
