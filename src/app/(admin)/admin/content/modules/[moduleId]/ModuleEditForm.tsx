...
type LessonFormState = Omit<
  Lesson,
  "createdAt" | "updatedAt" | "readingText"
> & {
  createdAt?: Date;
  updatedAt?: Date;
...
function SortableLessonItem({
  ...
  onTogglePremium: (id: string, status: boolean) => void;
  onUpdateType: (id: string, type: LessonType) => void;
  ...
  const getIcon = (type: LessonType) => {
    switch (type) {
      ...
      case "FLASHCARD":
        return <BrainCircuit size={18} className="text-orange-500" />;
      case "COMPLETION":
        return <Languages size={18} className="text-cyan-500" />;
      case "SPEAKING":
        return <Mic size={18} className="text-red-500" />;
      default:
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group flex flex-col sm:flex-row items-start sm:items-center justify-between bg-white border ${isDragging ? "border-interface-accent shadow-2xl" : "border-(--color-s-100)"} p-4 rounded-[24px] sm:rounded-[32px] hover:shadow-lg transition-all mb-3 gap-4 w-full`}
    >
      <div className="flex items-center gap-3 sm:gap-6 flex-1 w-full min-w-0">
        <div
          {...attributes}
          {...listeners}
          className="cursor-grab active:cursor-grabbing p-2 text-s-400 hover:text-s-600 transition-colors shrink-0"
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 group/input w-full">
            ...
            <LuPencil
              size={14}
              className="text-s-400 opacity-0 sm:group-hover/input:opacity-100 transition-opacity shrink-0"
            />
          </div>
          <div className="flex flex-wrap items-center gap-2 mt-1">
            <select
              value={lesson.type}
              onChange={(e) =>
                onUpdateType(lesson.id, e.target.value as LessonType)
              }
      ...
      <div className="flex items-center justify-end gap-2 w-full sm:w-auto border-t sm:border-t-0 pt-3 sm:pt-0 shrink-0">
        <button
          onClick={() => onExclude(lesson.id)}
          className="flex-1 sm:flex-none p-3 text-s-600 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all cursor-pointer flex justify-center"
        >
          <LuTrash2 size={20} />
        </button>
        {!String(lesson.id).startsWith("temp-") && (
          <Link
            href={`/admin/content/modules/${moduleId}/lessons/${lesson.id}`}
            className="flex-1 sm:flex-none p-3 bg-s-50 text-s-600 hover:text-s-900 rounded-xl transition-all flex justify-center"
          >
...
export function ModuleEditForm({ initialData }: { initialData: any }) {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const freshLessons = await (actions as any).syncModuleLessonsAction(
        initialData.id,
        lessons,
        deletedIds,
      );
      ...
      <div className="max-w-6xl mx-auto space-y-6 sm:space-y-10">
        ...
        >
          ...
          <button
            ...
            className="w-full py-4 mt-4 border-2 border-dashed rounded-[24px] sm:rounded-3xl text-s-500 hover:text-interface-accent hover:border-interface-accent transition-all flex items-center justify-center gap-2 font-black uppercase text-[10px] tracking-widest bg-white/50 active:scale-[0.98] cursor-pointer"
          >
