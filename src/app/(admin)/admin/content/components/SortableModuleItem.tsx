    ...
    >
      <div
        ...
        className={`group flex flex-col sm:flex-row items-stretch sm:items-center justify-between p-3 bg-white border rounded-xl transition-all gap-3 ${
          isDragging
            ? "border-interface-accent shadow-lg z-50"
            : "border-(--color-s-50) hover:border-(--color-s-100)"
        }
      `}
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing text-s-600 hover:text-s-800 shrink-0"
          >
            <GripVertical size={16} />
          </div>

          <div
            className={`p-2 rounded-lg shrink-0 ${lesson.isPremium ? "bg-s-50 text-s-800" : "bg-s-50 text-s-600"}`}
          >
            <Edit3 size={14} />
          </div>
  ...
  return (
    ...
    >
      <div
        ref={setNodeRef}
        style={style}
        className={`bg-white border-2 rounded-2xl overflow-hidden ${isDragging ? "border-interface-accent shadow-xl" : "border-(--color-s-50)"}`}
      >
          ...
          <div className="flex items-center gap-3 w-full sm:flex-1">
            ...
            <div className="flex-1 flex items-center gap-2 min-w-0">
              <input
                ...
                value={module.title}
                onChange={(e) =>
                  handleUpdateModuleName(module.id, e.target.value)
                }
              />
            ...
            {!mounted ? (
              <div className="space-y-2">
                {module.lessons?.map((lesson: any) => (
                  <div
                    key={lesson.id}
                    className="p-3 bg-white border rounded-xl opacity-50 text-s-600 text-xs"
                  >
                    {lesson.title}
                  </div>
                ))}
                  ...
                  <div className="space-y-2">
                    {module.lessons?.map((lesson: any) => (
                      <SortableLessonItem
                        ...
                        isMarkedForDeletion={itemsToDelete?.lessons.includes(
                          lesson.id,
                        )}
