...
interface SortableTrackItemProps {
  ...
  markForDeletion: (
    type: "track" | "module" | "lesson" | "objective",
    id: string,
  ) => void;
...
export function SortableTrackItem({
  ...
  return (
    <div
      className={`transition-all duration-300 rounded-4xl ${
        isMarkedForDeletion
          ? "opacity-50 grayscale bg-gray-200 pointer-events-none"
          : "bg-white"
      }`}
    >
      <div
        ref={setNodeRef}
        style={style}
        className={`bg-white border rounded-[2.5rem] overflow-hidden transition-all ${isDragging ? "z-50 border-interface-accent shadow-2xl" : "border-(--color-s-50)"}`}
      >
        <div
          className={`group p-6 flex flex-col md:flex-row items-stretch md:items-start gap-6 transition-all ${!track.active ? "bg-s-100" : "bg-white"}`}
        >
          <div className="flex flex-col md:inline-flex gap-4">
            <div
              {...attributes}
              {...listeners}
              className="px-2 flex items-center cursor-grab active:cursor-grabbing text-s-600 hover:text-s-800"
            >
            ...
            <div className="relative w-full md:w-60 h-48 md:h-80 shrink-0 rounded-[2rem] overflow-hidden group/img shadow-sm">
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    ...
                    reader.onloadend = () => {
                      ...
                      setLocalTracks((prev) =>
                        prev.map((t) =>
                          t.id === track.id ? { ...t, imageUrl: base64 } : t,
                        ),
                      );
                      setHasChanges(true);
                    };
              ...
              <img
                src={
                  track.imageUrl ||
                  "https://placehold.co/400x400/f8f9fa/666?text=+"
                }
          ...
          <div className="flex-1 space-y-4">
            <div className="flex flex-col md:flex-row items-start justify-between gap-4">
              ...
              <div className="flex flex-col gap-3 w-full md:w-auto">
                <div className="flex gap-2">
                  {!track.active ? (
                    <span className="bg-zinc-100 text-zinc-500 text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border border-zinc-200">
                      Rascunho
                    </span>
                  ) : (
                    <span className="bg-green-100 text-green-600 text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border border-green-200">
                      Online
                    </span>
                  )}
                </div>
                  ...
                  <button
                    onClick={() => {
                      if (
                        !track.active &&
                        !window.confirm(
                          "Ativar trilha imediatamente para os alunos?",
                        )
                      )
                        return;
                      setLocalTracks((prev) =>
                        prev.map((t) =>
                          t.id === track.id ? { ...t, active: !t.active } : t,
                        ),
                      );
                      setHasChanges(true);
                    }}
            ...
            <EditableDescription
              track={track}
              onValueChange={handleTrackDescriptionChange}
            />
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-4 border-t border-(--color-s-50)/50">
              <div className="flex flex-wrap gap-1.5 justify-start w-full">
                {plans?.map((plan) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${hasAccess ? "bg-white" : "bg-s-600"}`}
                      />
                      {plan.name}
                    </button>
                  );
              ...
              <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                ...
                <button
                  onClick={() => markForDeletion("track", track.id)}
                  className="p-3 text-s-600 rounded-xl hover:text-red-600 hover:bg-red-50 cursor-pointer"
                >
                  <LuTrash2 size={20} />
                </button>
                <button
                  ...
                  className="p-2 rounded-2xl bg-s-20 text-s-800 cursor-pointer"
                >
                  <ChevronDown
                    size={24}
