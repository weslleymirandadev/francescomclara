...
export function SortableObjectiveItem({
  o,
  activeObjectiveId,
  ...
  const handleColorChange = (color: string) => {
    setLocalObjectives((prev) =>
      prev.map((item) => (item.id === o.id ? { ...item, color: color } : item)),
    );
    setHasChanges(true);
  };
  ...
  const handleNameChange = (name: string) => {
    setLocalObjectives((prev) =>
      prev.map((item) => (item.id === o.id ? { ...item, name: name } : item)),
    );
    setHasChanges(true);
  };
    ...
    >
      <div
        className={`flex items-center w-full transition-all duration-300 ${
          isMarkedForDeletion ? "opacity-50 grayscale pointer-events-none" : ""
        }`}
      >
        <div
        ...
        >
          <div
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing text-s-400 hover:text-s-600"
          >
        ...
        <button
          ...
          className={`p-1 mr-2 rounded-lg transition-colors cursor-pointer ${
            isMarkedForDeletion
              ...
              : "text-s-300 hover:text-red-500 hover:bg-red-50"
          }`}
        >
