...
export async function updateTrackObjectiveAction(
  id: string,
  objectiveId: string,
) {
...
export async function updateObjectiveImageAction(
  id: string,
  imageBase64: string,
) {
  await prisma.objective.update({
...
export async function toggleModulePremiumAction(
  id: string,
...
export async function saveContentBulkAction(
  ...
  try {
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      const objectiveIdsToDelete = new Set<string>(
        itemsToDelete.objectives || [],
      );
      ...
      if (objectiveIdsToDelete.size > 0) {
        const realObjIds = Array.from(objectiveIdsToDelete).filter(
        ...
        if (realObjIds.length > 0) {
          ...
          if (relatedTrackIds.length > 0) {
            await tx.subscriptionPlanTrack.deleteMany({
              where: { trackId: { in: relatedTrackIds } },
            });
            await tx.track.deleteMany({
              where: { id: { in: relatedTrackIds } },
            });
          }
      ...
      if (trackIdsToDelete.size > 0) {
        const realTrackIds = Array.from(trackIdsToDelete).filter(
          (id) => !id.startsWith("temp-"),
        );
        if (realTrackIds.length > 0) {
          await tx.subscriptionPlanTrack.deleteMany({
            where: { trackId: { in: realTrackIds } },
          });
      ...
      for (const [objIndex, obj] of localObjectives.entries()) {
        if (new Set(itemsToDelete.objectives || []).has(obj.id)) continue;
        ...
        if (String(obj.id).startsWith("temp-")) {
          const created = await tx.objective.create({ data: objData });
          objIdMap.set(obj.id, created.id);
        } else {
          await tx.objective.update({
            where: { id: obj.id },
            data: objData,
          });
          objIdMap.set(obj.id, obj.id);
        }
      }
      ...
      for (const [tIndex, track] of localTracks.entries()) {
        ...
        if (track.modules) {
          for (const [mIndex, mod] of track.modules.entries()) {
            ...
            const savedMod = await tx.module.update({
              where: { id: mod.id },
              data: {
                title: mod.title,
    ...
    return {
      success: false,
      error: "Falha ao sincronizar com o banco de dados.",
    };
  }
}
  ...
  try {
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      if (deletedIds.length > 0) {
        await tx.lesson.deleteMany({
          where: {
            id: { in: deletedIds.filter((id) => !id.startsWith("temp-")) },
          },
        });
      }
