      ...
      <header className="mb-12 text-left">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
          <div className="flex justify-between items-end mb-3">
            ...
            <span className="text-lg font-black text-(--interface-accent)">
              {progressPercent}%
            </span>
          </div>
        ...
        <div className="mt-6">
          <Link href={`/flashcards?trackId=${track.id}`}>
            <Button
              variant="outline"
              className="w-full border-slate-200 hover:border-(--interface-accent) text-slate-600 hover:text-(--interface-accent) h-14 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all flex gap-3 bg-white"
            >
              <FaRegClone size={18} />
            ...
            <div key={module.id} className="relative">
              ...
              <div className="grid gap-4">
                {module.lessons?.map((lesson: any) => {
                  const isCompleted =
                    completedIds?.includes(lesson.id) || false;
                  ...
                  return (
                    <motion.div
                      ...
                      onClick={() =>
                        !isLocked &&
                        (window.location.href = `/dashboard/aula/${lesson.id}`)
                      }
                    ...
                    >
                      <div className="flex items-center gap-4 text-left">
                        ...
                        <div>
                          <h3
                            className={`font-bold ${isLocked ? "text-slate-400" : "text-slate-800"}`}
                          >
                            {lesson.title}
                          </h3>
                          {!isLocked && (
