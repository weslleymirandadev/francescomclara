...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setResumeTrack(contentData.trackToResume);
    } catch (error) {
        ...
        <div className="animate-in fade-in duration-700">
          {data?.profile?.onboarded === false && (
            <Card className="mb-12 border-none shadow-2xl bg-slate-900 rounded-[2.5rem] overflow-hidden relative">
              <div className="relative z-10 p-10 flex flex-col md:flex-row items-center justify-between gap-8 text-white">
                ...
                <Button
                  className="bg-(--interface-accent) text-white font-black uppercase text-[10px] tracking-widest rounded-2xl h-14 px-8"
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12">
            <Card className="p-6 border-none shadow-xl bg-white rounded-[2rem] flex flex-col justify-center items-center group hover:bg-(--interface-accent) transition-all">
              <FaBookOpen
                size={20}
                className="text-slate-400 mb-3 group-hover:text-white"
              />
              <h4 className="text-2xl font-black text-slate-900 group-hover:text-white">
                {data.enrollments.length}
              </h4>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest group-hover:text-white/80">
                Trilhas
              </p>
            </Card>
            ...
            <Card className="p-6 border-none shadow-xl bg-white rounded-[2rem] flex flex-col justify-center items-center group hover:bg-emerald-500 transition-all">
              ...
              <h4 className="text-2xl font-black text-slate-900 group-hover:text-white">
                {(() => {
                  const all = data.enrollments.flatMap((t) =>
                    t.modules.flatMap((m) => m.lessons),
                  );
                  const relevantLessons = all.filter(
                    (l) => l.type !== "FLASHCARD",
                  );
                  if (relevantLessons.length === 0) return "0%";
                  const done = relevantLessons.filter((l) =>
          ...
          {resumeTrack && (
            <section className="mb-12 bg-slate-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl">
              <div className="relative z-10">
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-(--interface-accent) mb-2 block">
                  {resumeTrack.progressPercent > 0
                    ? "Retomar Estudos"
                    : "Começar Nova Trilha"}
                </span>
                ...
                <div className="w-full max-w-md bg-white/10 h-2 rounded-full mb-8 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${resumeTrack.progressPercent}%` }}
                  />
                </div>

                <Button
                  onClick={() => router.push(`/curso/${resumeTrack.id}`)}
          ...
          <div className="mb-8 px-2">
            ...
            <p className="text-lg font-black text-slate-800 uppercase tracking-tighter leading-none">
              Explorar Trilhas
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
            {filteredTracks.length === 0 ? (
              ...
              filteredTracks.map((track: Track) => {
                const hasAccess =
                  hasAllAccess ||
                  userPlanFeatures.includes(`track:${track.id}`) ||
                  track.hasAccess;
                const allTrackLessons =
                  track.modules?.flatMap((m: any) => m.lessons) || [];

                const filteredTrackLessons = allTrackLessons.filter(
                  (l: any) => l.type !== "FLASHCARD",
                );
                ...
                const trackProgress =
                  filteredTrackLessons.length > 0
                    ? Math.round(
                        (completedInTrack / filteredTrackLessons.length) * 100,
                      )
                    : 0;
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => {
                      if (!hasAccess) {
                        toast.error(
                          "Esta trilha não está incluída no seu plano atual.",
                        );
                        router.push("/assinar");
                        return;
                      }
                  ...
                  >
                    ...
                    <div className="p-8 text-left">
                      ...
                      <div className="space-y-3 mb-6">
                        ...
                        {(track.modules?.[0]?.lessons?.length > 3 ||
                      ...
                      <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                        <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase">
                          <FaBookOpen className="text-(--interface-accent)" />{" "}
                          {track.modules?.length || 0} Módulos
                        </div>
                        {hasAccess && (
