...
export default function FlashcardsPage() {
  ...
  return (
    <main className="min-h-screen bg-s-50 pt-24 pb-12 px-6">
      <div className="max-w-xl mx-auto">
        
        {mode === 'IDLE' && (
          <div className="bg-white p-10 rounded-3xl shadow-sm border border-(--color-s-200) text-center">
            ...
            <h1 className="text-2xl font-bold text-s-800 mb-2">Revisão Diária</h1>
            <p className="text-s-500 mb-8">Tens {cards.length} flashcards para praticar hoje.</p>
            <button 
        ...
        {mode === 'STUDY' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between px-2">
              <button onClick={() => setMode('IDLE')} className="text-s-400 hover:text-s-600">
                <FiArrowLeft size={24} />
              </button>
              <div className="text-sm font-bold text-s-400">
            ...
            >
              ...
              >
                <div className="absolute inset-0 backface-hidden bg-white rounded-3xl shadow-xl border-2 border-(--color-s-100) flex flex-col items-center justify-center p-8">
                  <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mb-4">Francês</span>
                  <h2 className="text-4xl font-bold text-s-800">{cards[currentIndex].front}</h2>
                  <p className="mt-8 text-s-300 text-xs font-medium">Clica para ver a tradução</p>
                </div>
                ...
                >
                  ...
                  <h2 className="text-3xl font-medium text-s-700">{cards[currentIndex].back}</h2>
        ...
        {mode === 'DONE' && (
          <div className="bg-white p-10 rounded-3xl shadow-sm border border-(--color-s-200) text-center animate-in fade-in zoom-in duration-500">
            <div className="text-5xl mb-6">🌸</div>
            <h1 className="text-2xl font-bold text-s-800 mb-2">Excelente Trabalho!</h1>
            <p className="text-s-500 mb-8">Revisaste {cards.length} termos com sucesso.</p>
            
            <div className="flex bg-s-50 p-1 rounded-2xl mb-8">
            ...
            <button 
              ...
              className="w-full py-4 bg-s-900 text-white font-bold rounded-2xl hover:bg-s-800"
            >
              Finalizar Revisão
            </button>
          </div>
